...
void CDlgStat::OnPaint()
{
    ...
    // 将客户区坐标转换为对话框坐标
    m_staticPie.MapWindowPoints(this, &rect);
    ...
    // 绘制饼图
    DrawPie(&memDC, rect, m_cateAmount);
...
{
    // 计算饼图绘制区域（正方形，位于控件中央）
    int pieSize = min(rect.Width(), rect.Height()) - 40;
    if (pieSize < 50) pieSize = 50;
    int x = (rect.Width() - pieSize) / 2;
    int y = (rect.Height() - pieSize) / 2;
    ...
    if (total == 0)
    {
        ...
        pDC->TextOut(x + pieSize / 2 - 40, y + pieSize / 2 - 10, L"暂无支出数据");
    ...
    for (int i = 0; i < 6; i++)
    {
        if (cateAmount[i] > 0)
        {
            ...
            CBrush brush(colors[i]);
            CBrush* pOldBrush = pDC->SelectObject(&brush);
            pDC->Rectangle(legendX, legendY, legendX + 15, legendY + 15);
            pDC->SelectObject(pOldBrush);
            brush.DeleteObject();
            ...
            pDC->SetBkMode(TRANSPARENT);
            pDC->TextOut(legendX + 20, legendY, text);
